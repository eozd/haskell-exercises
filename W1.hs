...
fizzbuzz num = firstPart ++ secondPart
    where firstPart = if num `mod` 3 == 0 then "Fizz" else ""
          secondPart = if num `mod` 5 == 0 then "Buzz" else ""
...
tribonacci 2 = 1
tribonacci 3 = 2
tribonacci n = sumTri 3 n 1 1 2
    where sumTri curr tot a b c = if curr == tot
                                  then c
                                  else sumTri (curr + 1) n b c (a + b + c)
...
pyramid 0 = "0"
pyramid n = leftPart ++ show n ++ rightPart
    where leftPart = incrStr (n - 1)
          rightPart = decrStr (n - 1)
          incrStr 0 = "0,"
          incrStr n = incrStr (n - 1) ++ show n ++ ","
          decrStr 0 = ",0"
          decrStr n = "," ++ show n ++ decrStr (n - 1)
...
smallestDivisor n = checkDiv n 2
    where checkDiv n k = if n `mod` k == 0 then k else checkDiv n (k + 1)
